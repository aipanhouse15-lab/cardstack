...
const SvgPremiumVsAge = () => (
  <svg viewBox="0 0 720 286" role="img" aria-label="Health insurance annual premium versus age showing how premium jumps significantly at age 55 60 and 65 for a 5 lakh sum insured policy" style={{ width: "100%", maxWidth: 760, margin: "24px 0" }}>
    ...
    {[{ age: "35", premium: 8500, x: 100 }, { age: "45", premium: 14200, x: 200 }, { age: "50", premium: 19800, x: 300 }, { age: "55", premium: 28500, x: 400 }, { age: "60", premium: 42000, x: 500 }, { age: "65", premium: 58000, x: 600 }, { age: "70", premium: 72000, x: 700 }].map(({ age, premium, x }, i) => (
      <g key={age}>
        ...
        <text x="20" y="252" fontSize="9" fontFamily="system-ui" fill="var(--text-muted)">Premium at age 60 is 5x that at age 35 for same coverage. At 70, it's 8.5x. This is why buying health insurance for</text>
    <text x="20" y="266" fontSize="9" fontFamily="system-ui" fill="var(--text-muted)">parents when they're in their 50s — before pre-existing conditions set in — is critical.</text>
  </svg>
);

const SvgInsurerSeniorComparison = () => (
  <svg viewBox="0 0 720 317" role="img" aria-label="Health insurance comparison for 60 year old parents across Star Health Niva Bupa HDFC Ergo Care Health and New India showing premium waiting period and entry age limit" style={{ width: "100%", maxWidth: 760, margin: "24px 0" }}>
    ...
    {[
      ...
      <g key={row[0]}>
        ...
        <text x="20" y="255" fontSize="9" fontFamily="system-ui" fill="var(--text-muted)">Premiums are approximate for a couple, age 60-62, non-smoking, ₹5L SI, no major PEDs. Actual premiums vary — always get</text>
    <text x="20" y="269" fontSize="9" fontFamily="system-ui" fill="var(--text-muted)">quotes for your parents' specific age and health profile.</text>
  </svg>
);

const SvgWaitingPeriod = () => (
  <svg viewBox="0 0 720 250" role="img" aria-label="Pre-existing disease waiting period comparison showing that diseases like diabetes hypertension and heart conditions have 2 to 4 year waiting periods before insurance pays for related claims" style={{ width: "100%", maxWidth: 760, margin: "24px 0" }}>
    ...
    {[
      ...
      <g key={condition}>
        ...
        <text x="20" y="215" fontSize="9" fontFamily="system-ui" fill="var(--text-muted)">Buy before conditions are diagnosed. Once your parent has diabetes or hypertension documented, it's a PED and has a</text>
    <text x="20" y="229" fontSize="9" fontFamily="system-ui" fill="var(--text-muted)">waiting period at every new insurer. Existing insurers retain the old waiting period on renewal.</text>
  </svg>
);

const SvgSuperTopUpMath = () => (
  <svg viewBox="0 0 720 269" role="img" aria-label="Super top-up insurance math showing how a 5 lakh base policy plus 20 lakh super top-up gives 25 lakh coverage for much lower total premium than buying 25 lakh directly" style={{ width: "100%", maxWidth: 760, margin: "24px 0" }}>
    ...
    <text x="360" y="216" textAnchor="middle" fontFamily="system-ui" fontSize="13" fontWeight="700" fill="#fff">Super top-up saves ₹53,000/year = ₹5.3L over 10 years for same coverage</text>
        <text x="20" y="235" fontSize="9" fontFamily="system-ui" fill="var(--text-muted)">Super top-up kicks in only when a single hospitalization crosses the deductible (₹5L here). Perfect for catastrophic</text>
    <text x="20" y="249" fontSize="9" fontFamily="system-ui" fill="var(--text-muted)">coverage. Base policy handles routine hospitalizations.</text>
  </svg>
);

const SvgFlaterVsIndividual = () => (
  <svg viewBox="0 0 720 229" role="img" aria-label="Floater versus individual health insurance policy comparison for parents showing that individual policies are better for older parents with different health profiles" style={{ width: "100%", maxWidth: 760, margin: "24px 0" }}>
    ...
    <text x="540" y="155" textAnchor="middle" fontFamily="system-ui" fontSize="11" fill="var(--text-muted)">Recommended for parents above 65</text>
        <text x="20" y="195" fontSize="9" fontFamily="system-ui" fill="var(--text-muted)">For parents above 65 or with different existing conditions, individual policies are strongly recommended. The peace of</text>
    <text x="20" y="209" fontSize="9" fontFamily="system-ui" fill="var(--text-muted)">mind from separate coverage outweighs the ~20% premium premium.</text>
  </svg>
);
