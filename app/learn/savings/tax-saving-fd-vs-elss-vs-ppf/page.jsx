...
const SvgReturns5yr = () => (
  <svg viewBox="0 0 720 280" role="img" aria-label="Post-tax 5 year returns comparison between tax saving FD ELSS and PPF on 1.5 lakh annual investment showing ELSS at 11.2 lakh PPF at 8.9 lakh and tax FD at 7.8 lakh" style={{ width: "100%", maxWidth: 760, margin: "24px 0" }}>
    ...
    <text x="20" y="295" fontFamily="system-ui" fontSize="10" fill="var(--text-muted)">ELSS can vary: in bad 5-year windows (2018-2023), returns were 6-8%. PPF is guaranteed. Tax FD has the guaranteed lowest post-tax return of the three for 30% slab investors.</text>
  </svg>
);

const SvgLockIn = () => (
  <svg viewBox="0 0 720 180" role="img" aria-label="Lock-in period comparison showing ELSS at 3 years tax saving FD at 5 years and PPF at 15 years with explanation of what happens after lock-in" style={{ width: "100%", maxWidth: 760, margin: "24px 0" }}>
    ...
    <text x="20" y="160" fontFamily="system-ui" fontSize="10" fill="var(--text-muted)">ELSS wins on flexibility — 3-year lock-in makes it the most accessible 80C equity option. But it also carries the most risk. Short lock-in + market volatility can mean lower returns if your exit is timed badly.</text>
    <text x="20" y="173" fontFamily="system-ui" fontSize="10" fill="var(--text-muted)">After lock-in: ELSS and Tax FD proceeds are yours to redeploy. PPF can be extended 5 years at a time — you continue to earn EEE returns without depositing more (or continue depositing up to ₹1.5L/yr).</text>
  </svg>
);

const SvgRiskReturnPlot = () => (
  <svg viewBox="0 0 720 240" role="img" aria-label="Risk versus post-tax return plot for ELSS PPF and tax saving FD showing ELSS has highest risk and return PPF has no risk and moderate return and tax FD has no risk but lowest return" style={{ width: "100%", maxWidth: 760, margin: "24px 0" }}>
    ...
    <text x="20" y="235" fontFamily="system-ui" fontSize="10" fill="var(--text-muted)">Circle size = liquidity (PPF is mid, ELSS is most liquid after 3yr, Tax FD is illiquid for 5yr). Tax FD earns less than PPF with identical safety. Avoid Tax FD unless ELSS and PPF are already maxed.</text>
  </svg>
);
...
const SvgDecisionFramework = () => (
  <svg viewBox="0 0 720 200" role="img" aria-label="Decision framework for 80C investment showing which instrument to pick based on risk tolerance income level and time horizon" style={{ width: "100%", maxWidth: 760, margin: "24px 0" }}>
    ...
    <text x="20" y="192" fontFamily="system-ui" fontSize="10" fill="var(--text-muted)">Tax FD earns less than PPF with the same lock-in safety. The only reason to pick Tax FD: EPF/insurance already fills your 80C bucket and you want a guaranteed, capital-safe overflow option.</text>
  </svg>
);

const SvgELSSFundPicks = () => (
  <svg viewBox="0 0 720 180" role="img" aria-label="Top ELSS mutual fund picks for 80C investment showing 5 year returns for Mirae Asset Tax Saver Axis Long Term Equity and Canara Robeco ELSS Tax Saver" style={{ width: "100%", maxWidth: 760, margin: "24px 0" }}>
    ...
    <text x="20" y="173" fontFamily="system-ui" fontSize="10" fill="var(--text-muted)">Past performance isn't guaranteed. Prefer direct plans (Zerodha Coin, Kuvera, MF Central) over regular for same fund. 5-year windows can be misleading — check 10yr and 15yr data too.</text>
  </svg>
);
