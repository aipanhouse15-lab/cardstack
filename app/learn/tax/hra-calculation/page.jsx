...
const SvgHRAFormula = () => (
  <svg viewBox="0 0 720 424" role="img" aria-label="HRA exemption formula showing it is minimum of three values: actual HRA received, 50 percent of basic plus DA for metro or 40 percent for non-metro, and actual rent paid minus 10 percent of basic plus DA" style={{ width: "100%", maxWidth: 760, margin: "24px 0" }}>
...
const SvgCityExamples = () => (
  <svg viewBox="0 0 720 586" role="img" aria-label="HRA calculation examples for Rahul in Mumbai Priya in Hyderabad and Amit in Pune showing step by step calculation with different basic salaries and rent amounts" style={{ width: "100%", maxWidth: 760, margin: "24px 0" }}>
    ...
    {[
      ...
      <g key={name}>
        ...
        <text x="20" y="275" fontSize="9" fontFamily="system-ui" fill="var(--text-muted)">Rahul and Priya's exempt HRA is limited by component C (rent - 10% of basic). This is why paying more rent (within</text>
    <text x="20" y="289" fontSize="9" fontFamily="system-ui" fill="var(--text-muted)">reason) increases HRA exemption. But only if rent receipts are documented.</text>
  </svg>
);

const SvgDocumentation = () => (
  <svg viewBox="0 0 720 264" role="img" aria-label="HRA documentation requirements showing rent receipts PAN of landlord if rent above 1 lakh and rental agreement required to claim HRA exemption" style={{ width: "100%", maxWidth: 760, margin: "24px 0" }}>
    ...
    {[
      ...
      <g key={doc}>
        ...
        <text x="20" y="194" fontSize="9" fontFamily="system-ui" fill="var(--text-muted)">Always pay rent via bank transfer (NEFT/UPI). Cash payments are harder to prove in an IT assessment. Keep 3 years of rent</text>
    <text x="20" y="208" fontSize="9" fontFamily="system-ui" fill="var(--text-muted)">receipts and rental agreement even after changing residence.</text>
  </svg>
);

const SvgHRAvsHomeLoan = () => (
  <svg viewBox="0 0 720 224" role="img" aria-label="Comparison of claiming HRA exemption versus home loan deductions showing that both can be claimed simultaneously if you rent in one city and own property in another" style={{ width: "100%", maxWidth: 760, margin: "24px 0" }}>
    ...
    <text x="30" y="58" fontFamily="system-ui" fontSize="13" fontWeight="700" fill={COLOR}>YES — if you rent in the city you work and own property in a different city.</text>
        <text x="30" y="78" fontSize="9" fontFamily="system-ui" fill="var(--text)">Example: Neha owns a flat in Nagpur (rented out, no self-occupation) but lives and works in Pune on rent. She can claim</text>
    <text x="30" y="92" fontSize="9" fontFamily="system-ui" fill="var(--text)">HRA on Pune rent AND Section 24(b) on home loan interest of her Nagpur property.</text>
    ...
    <text x="30" y="120" fontFamily="system-ui" fontSize="13" fontWeight="700" fill="#dc2626">NOT ALLOWED — if you own property in the SAME city you're claiming HRA for.</text>
        <text x="30" y="140" fontSize="9" fontFamily="system-ui" fill="var(--text)">Example: Owning a flat in Bangalore but renting nearby to avoid commute. The IT department can question the need for</text>
    <text x="30" y="154" fontSize="9" fontFamily="system-ui" fill="var(--text)">renting — you'll need to show genuine reason (under construction, occupied by parents, etc.).</text>
        <text x="20" y="190" fontSize="9" fontFamily="system-ui" fill="var(--text-muted)">Always consult a CA for the same-city situation. The IT department scrutinizes HRA + home loan combinations in the same</text>
    <text x="20" y="204" fontSize="9" fontFamily="system-ui" fill="var(--text-muted)">city closely. Genuine scenarios (parental occupation, under-construction flat) are generally accepted.</text>
  </svg>
);

const SvgTaxSavedByCity = () => (
  <svg viewBox="0 0 720 265" role="img" aria-label="Annual tax saved from HRA exemption by city for different salary and rent levels showing Mumbai saving most due to metro 50 percent formula" style={{ width: "100%", maxWidth: 760, margin: "24px 0" }}>
    ...
    {[
      ...
      <g key={city}>
        ...
        <text x="20" y="193" fontSize="9" fontFamily="system-ui" fill="var(--text-muted)">Metro city workers save significantly more from HRA due to the 50% formula vs 40%. This makes HRA one of the largest</text>
    <text x="20" y="207" fontSize="9" fontFamily="system-ui" fill="var(--text-muted)">deductions for salaried employees in metro cities — don't leave it unclaimed.</text>
  </svg>
);

const SvgRentFamilyTrap = () => (
  <svg viewBox="0 0 720 186" role="img" aria-label="Paying rent to parents to claim HRA showing rules and requirements including that parents must declare rental income and PAN is required if rent exceeds 1 lakh annually" style={{ width: "100%", maxWidth: 760, margin: "24px 0" }}>
    ...
    <text x="30" y="58" fontFamily="system-ui" fontSize="13" fontWeight="700" fill={COLOR}>YES — paying rent to parents to claim HRA is legal and accepted by IT department.</text>
        <text x="30" y="76" fontSize="9" fontFamily="system-ui" fill="var(--text)">Conditions: (1) Property must be in parent's name, not yours. (2) Genuine rental agreement signed. (3) Rent paid via bank</text>
    <text x="30" y="90" fontSize="9" fontFamily="system-ui" fill="var(--text)">transfer (not cash). (4) Parents must declare rental income in their ITR.</text>
        <text x="30" y="94" fontSize="9" fontFamily="system-ui" fill="var(--text)">Parent's income from rent: taxable. But if parents are in nil or 5% slab, their tax on rental income is minimal or zero.</text>
    <text x="30" y="108" fontSize="9" fontFamily="system-ui" fill="var(--text)">The family saves net tax overall.</text>
        <text x="30" y="112" fontSize="9" fontFamily="system-ui" fill="var(--text)">Landlord PAN required: if annual rent exceeds ₹1L. Since parents own property, they provide their PAN to you (their child</text>
    <text x="30" y="126" fontSize="9" fontFamily="system-ui" fill="var(--text)">tenant). This is perfectly legal — it's a bonafide landlord-tenant arrangement.</text>
        <text x="20" y="152" fontSize="9" fontFamily="system-ui" fill="var(--text-muted)">Common mistake: stopping monthly bank transfers and submitting cash receipts. Always maintain a consistent monthly</text>
    <text x="20" y="166" fontSize="9" fontFamily="system-ui" fill="var(--text-muted)">payment trail. Irregular or year-end bulk payments are questioned in assessments.</text>
  </svg>
);
