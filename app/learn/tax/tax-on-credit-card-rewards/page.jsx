...
const SvgTaxabilityFlowchart = () => (
  <svg viewBox="0 0 720 324" role="img" aria-label="Flowchart showing when credit card reward points are taxable including business cards cash rewards over 50000 and when they are not taxable including personal card points redeemed for merchandise" style={{ width: "100%", maxWidth: 760, margin: "24px 0" }}>
    ...
    <text x="400" y="250" textAnchor="middle" fontFamily="system-ui" fontSize="10" fill="#fff">as income from other sources</text>
        <text x="20" y="290" fontSize="9" fontFamily="system-ui" fill="var(--text-muted)">CBDT's position: rewards from spend are "discount/rebate" not income. Gifts/prizes are income. The ₹50K threshold is from</text>
    <text x="20" y="304" fontSize="9" fontFamily="system-ui" fill="var(--text-muted)">Section 56(2)(x) — gifts above ₹50K from non-relatives are taxable.</text>
  </svg>
);

const SvgRewardTypeMatrix = () => (
  <svg viewBox="0 0 720 269" role="img" aria-label="Matrix of different credit card reward types and their tax treatment including points for spend cashback welcome bonus referral rewards and milestone benefits" style={{ width: "100%", maxWidth: 760, margin: "24px 0" }}>
    ...
    {[
      ...
      <g key={row[0]}>
        ...
        <text x="20" y="235" fontSize="9" fontFamily="system-ui" fill="var(--text-muted)">CBDT has not issued specific circulars on every scenario. The "discount vs income" distinction is the operative</text>
    <text x="20" y="249" fontSize="9" fontFamily="system-ui" fill="var(--text-muted)">principle. When in doubt, consult a CA — especially for rewards above ₹50K annually.</text>
  </svg>
);

const SvgHighValueRedemption = () => (
  <svg viewBox="0 0 720 254" role="img" aria-label="High value credit card redemption risk zones showing scenarios where IT department might scrutinize large reward redemptions especially on business cards" style={{ width: "100%", maxWidth: 760, margin: "24px 0" }}>
    ...
    {[
      ...
      <g key={scenario}>
        ...
        <text x="20" y="195" fontSize="9" fontFamily="system-ui" fill="var(--text-muted)">The safest position: treat all credit card rewards as tax-free (current CBDT interpretation). The risky position: large</text>
    <text x="20" y="209" fontSize="9" fontFamily="system-ui" fill="var(--text-muted)">referral earnings, business card cashback, or contest prizes — declare these as income from other sources.</text>
  </svg>
);

const SvgPracticalAdvice = () => (
  <svg viewBox="0 0 720 209" role="img" aria-label="Practical tax advice for credit card reward holders showing what to declare what is safe to ignore and what needs CA consultation" style={{ width: "100%", maxWidth: 760, margin: "24px 0" }}>
    ...
    {["Referral cash rewards above ₹5,000 (cumulative)", "Contest/lucky draw prizes from card company", "Business card cashback (belongs to employer)", "Points sold or transferred for cash"].map((t, i) => (
      <text key={t} x="390" y={80 + i * 18} fontFamily="system-ui" fontSize="10" fill="var(--text)">✗ {t}</text>
        ...
        <text x="20" y="175" fontSize="9" fontFamily="system-ui" fill="var(--text-muted)">Middle ground: if your annual reward accumulation (all forms) exceeds ₹50K, get a CA to review. Below ₹50K from personal</text>
    <text x="20" y="189" fontSize="9" fontFamily="system-ui" fill="var(--text-muted)">card spend: current practice is to not declare, aligned with CBDT's "discount" interpretation.</text>
  </svg>
);

const SvgBusinessCardTax = () => (
  <svg viewBox="0 0 720 204" role="img" aria-label="Business credit card reward tax treatment showing that rewards on company expenses belong to the company not the employee and must be declared as business income or employee perquisite" style={{ width: "100%", maxWidth: 760, margin: "24px 0" }}>
    ...
    <rect x="20" y="38" width="680" height="120" fill="var(--raise)" stroke="#dc2626" strokeWidth="1" rx="8" />
        <text x="30" y="60" fontSize="9" fontFamily="system-ui" fontWeight="700" fill="#dc2626">Situation: Employee uses company credit card for business expenses, accumulates 2L reward points, redeems for personal</text>
    <text x="30" y="74" fontSize="9" fontFamily="system-ui" fontWeight="700" fill="#dc2626">travel.</text>
        <text x="30" y="82" fontSize="9" fontFamily="system-ui" fill="var(--text)">Tax position: Points accumulated on company spending are technically the company's assets. When an employee personally</text>
    <text x="30" y="96" fontSize="9" fontFamily="system-ui" fill="var(--text)">redeems them, it's a perquisite — taxable as salary.</text>
        <text x="30" y="100" fontSize="9" fontFamily="system-ui" fill="var(--text)">How it's actually handled: Almost no Indian company tracks this. Card companies don't issue Form 26AS or TDS certificates</text>
    <text x="30" y="114" fontSize="9" fontFamily="system-ui" fill="var(--text)">for reward redemptions.</text>
        <text x="30" y="118" fontSize="9" fontFamily="system-ui" fill="var(--text)">Safe approach: If your employer gives you a corporate card, clarify their policy on personal redemption of business card</text>
    <text x="30" y="132" fontSize="9" fontFamily="system-ui" fill="var(--text)">rewards. Some companies have explicit policies allowing or prohibiting this.</text>
        <text x="30" y="136" fontSize="9" fontFamily="system-ui" fontWeight="600" fill="#dc2626">Practical reality: This is a grey area. The risk of scrutiny is low for moderate amounts. For frequent flyer points worth</text>
    <text x="30" y="150" fontSize="9" fontFamily="system-ui" fontWeight="600" fill="#dc2626">₹5L+ redeemed annually: consult a CA.</text>
        <text x="20" y="170" fontSize="9" fontFamily="system-ui" fill="var(--text-muted)">Companies with formal expense reimbursement policies (travel cards with direct billing) don't create this issue — the</text>
    <text x="20" y="184" fontSize="9" fontFamily="system-ui" fill="var(--text-muted)">card company invoices the company directly and employees don't accumulate personal points.</text>
  </svg>
);
