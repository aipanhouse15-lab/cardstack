...
export default function CompareClient() {
  ...
  const selStyle = { width: "100%", background: "var(--bg-input)", border: "1px solid var(--border)", borderRadius: 10, padding: "12px 14px", color: "var(--text)", fontSize: 14, fontFamily: "inherit", cursor: "pointer" };
      ...
      <div className="grid grid-cols-2 gap-4 mb-8">
        {[{ v: id1, s: setId1, l: "Card 1" }, { v: id2, s: setId2, l: "Card 2" }].map((x, i) => (
          <div key={i}>
            ...
            <select value={x.v} onChange={e => x.s(e.target.value)} style={selStyle}>
      ...
      {c1 && c2 && (
        <div className="animate-fade-up">
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            {[c1, c2].map((c, i) => (
              <Link key={i} href={`/cards/${c.id}`} className="rounded-xl p-5 text-center no-underline transition-all hover:shadow-md cursor-pointer"
                ...
                <div className="flex justify-center gap-3 mt-2.5 text-[11px]">
                ...
                <div className="text-[10px] mt-1.5" style={{ color: "var(--text-faint)" }}>Tap for full details</div>
              </Link>
            ))}
          </div>
          ...
          <div className="rounded-2xl overflow-hidden" style={{ background: "var(--bg-card)", border: "1px solid var(--border)", boxShadow: "var(--shadow)" }}>
            <div className="grid grid-cols-[1.2fr_1fr_1fr] px-4 py-3" style={{ background: "var(--bg-muted)", borderBottom: "1px solid var(--border)" }}>
            ...
            {CATEGORIES.map((cat, i) => {
              ...
              return (
                <div key={cat.id} className="grid grid-cols-[1.2fr_1fr_1fr] px-4 py-3" style={{ borderBottom: i < CATEGORIES.length - 1 ? "1px solid var(--border-light)" : "none" }}>
          ...
          {(() => {
            let s1 = 0, s2 = 0;
            CATEGORIES.forEach(cat => { const r1 = c1.rewards[cat.id] || c1.rewards.default; const r2 = c2.rewards[cat.id] || c2.rewards.default; if (r1 > r2) s1++; else if (r2 > r1) s2++; });
            return (
              <div className="grid grid-cols-2 gap-4 mt-4">
                {[[s1, c1.name, s1 >= s2], [s2, c2.name, s2 > s1]].map(([score, name, win], i) => (
                  <div key={i} className="rounded-xl p-4 text-center"
                    style={{ background: win ? "var(--green-bg)" : "var(--bg-muted)", border: `1px solid ${win ? "var(--green-border)" : "var(--border)"}` }}>
                    <div className="text-3xl font-extrabold font-mono" style={{ color: win ? "var(--green)" : "var(--text-muted)" }}>{score}</div>
                    <div className="text-xs" style={{ color: "var(--text-muted)" }}>categories won</div>
                    <div className="text-sm font-semibold mt-1" style={{ color: win ? "var(--green)" : "var(--text-muted)" }}>{name}</div>
                  </div>
                ))}
              </div>
            );
