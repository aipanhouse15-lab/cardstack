...
function getCategoryWinners() {
  const free = CARDS.filter(c => c.fee === 0).sort((a, b) => Math.max(...Object.values(b.rewards)) - Math.max(...Object.values(a.rewards)));
  const bestForCategory = (catId) => [...CARDS].sort((a, b) => (b.rewards[catId] || 0) - (a.rewards[catId] || 0)).slice(0, 4);
  ...
  const cashback = CARDS.filter(c => c.type === "Cashback" || c.rewards.online >= 3).sort((a, b) => Math.max(...Object.values(b.rewards)) - Math.max(...Object.values(a.rewards)));
  return {
    free: free.slice(0, 5), dining: bestForCategory("dining"), travel: bestForCategory("travel"),
    online: bestForCategory("online"), fuel: CARDS.filter(c => c.rewards.fuel > 0).sort((a, b) => b.rewards.fuel - a.rewards.fuel).slice(0, 4),
...
export default function CardsPage() {
  ...
  const catalogSchema = {
    ...
    description: "Compare 75 Indian credit cards with cap-adjusted cashback math.",
