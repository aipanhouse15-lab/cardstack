...
const IconLounge = () => (
  <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Airport lounge icon">
    ...
    <circle cx="14" cy="22" r="3" fill={COLOR} opacity="0.5"/>
    <path d="M8 36 L40 36" stroke={COLOR} strokeWidth="1" strokeDasharray="3 2"/>
  </svg>
);
...
const IconTrophy = () => (
  <svg width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Top ranked card trophy">
    ...
    <rect x="11" y="20" width="6" height="3" rx="1" fill={COLOR} opacity="0.5"/>
    <rect x="9" y="23" width="10" height="2" rx="1" fill={COLOR}/>
  </svg>
);
...
const IconGlobe = () => (
  <svg width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="International lounge access">
    ...
    <path d="M5 9H23M5 19H23" stroke={COLOR} strokeWidth="1" strokeDasharray="2 2"/>
  </svg>
);
...
export default function BestCreditCardForLoungeAccess() {
  const faq = {
  ...
  return (
    <main style={{ maxWidth: 800, margin: "0 auto", padding: "32px 22px 48px", fontFamily: "system-ui, -apple-system, sans-serif", color: "var(--text)", lineHeight: 1.65 }}>
      ...
      <nav style={{ fontSize: 13, color: "var(--text-muted,#64748b)", marginBottom: 24 }}>
        ...
        <span>Best Credit Card for Lounge Access</span>
      </nav>
          ...
          {visitValueTable.map(({ profile, rec, note }, i) => (
            <div key={i} style={{ display: "flex", gap: 12, alignItems: "flex-start", padding: "10px 12px", background: "var(--raise)", borderRadius: 8 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <span style={{ color: COLOR, fontWeight: 700, fontSize: 14 }}> {rec}.</span>
                <span style={{ color: "var(--text-muted,#64748b)", fontSize: 13 }}> {note}</span>
              </div>
            </div>
          ))}
        </div>
        ...
        {cards.map((card) => (
          <div key={card.rank} style={{ border: `2px solid ${card.rank === 1 ? COLOR : "var(--hair)"}`, borderRadius: 14, padding: "20px 22px", background: card.rank === 1 ? `${COLOR}06` : "transparent" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, fontSize: 13, color: "var(--text-muted,#475569)", marginBottom: 12 }}>
              <div><span style={{ fontWeight: 600 }}>Domestic:</span> {card.domestic}</div>
              <div><span style={{ fontWeight: 600 }}>International:</span> {card.intl}</div>
              <div><span style={{ fontWeight: 600 }}>Guest Pass:</span> {card.guestPass}</div>
              <div>
                <span style={{ fontWeight: 600 }}>Spend Condition:</span>{" "}
                <span style={{ color: card.spendCondition === "None" || card.spendCondition === "None post-April 2026" ? "#16a34a" : "#f97316" }}>{card.spendCondition}</span>
              </div>
              <div style={{ gridColumn: "1/-1" }}><span style={{ fontWeight: 600 }}>Who Can Apply:</span> {card.access}</div>
            </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 28 }}>
        {[
          { step: "Download the DreamFolks app and register your card before your first visit. Some banks require pre-activation — doing this at the airport entry gate causes delays.",
            icon: "1" },
          { step: "For Priority Pass, download the Priority Pass app and link your membership number. The physical card takes 2-3 weeks to arrive after card issuance.",
            icon: "2" },
          { step: "Check visit limits at the start of each quarter if your card has spend-gated access. Log in to your bank app and confirm the benefit is active before travelling.",
            icon: "3" },
      ...
      <div style={{ background: `${COLOR}08`, border: `2px solid ${COLOR}`, borderRadius: 16, padding: "24px 26px", marginBottom: 44 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
          <IconPlane />
          <h3 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: COLOR }}>Plan Your Travel Card Stack</h3>
        </div>
